...
<template>
  <div
    class="h-screen md:flex items-center justify-center gap-0 flex-row-reverse bg-bgOverlay overflow-hidden"
  >
    ...
    <div
      ref="loginRef"
      class="md:w-1/2 h-full bg-bgOverlay px-4 md:px-16 xl:px-28 2xl:px-60 pt-10 relative"
    >
      <div
      ...
      <div class="mt-6">
        <form @submit="onSubmit">
          <div
            class="flex w-full items-center justify-between md:gap-[30px] flex-col md:flex-row"
          >
            <Input
              name="fullName"
              type="text"
              placeholder="Full Name"
              extend-class-wrapper="w-full"
            />
            <Input
              name="phone"
              type="text"
          ...
          >
            ...
            <RouterLink
              :to="path.login"
              class="block font-medium text-[16px] mt-4 lg:mt-0 text-white opacity-70 transition-colors duration-200 hover:text-[#fa2851]"
              >Already registered! Login here?</RouterLink
            >
          </div>
        </form>
      </div>
